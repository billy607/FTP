...
EVP_PKEY* CryptoImpl::localKeypair;
...
CryptoImpl::CryptoImpl() {
  ...
  remotePublicKey = NULL;
    ...
    buffer = new char[bufferSize] ;//8m 文件大小


  init();
}

CryptoImpl::CryptoImpl(unsigned char *remotePublicKey, size_t remotePublicKeyLength) {
  localKeypair = NULL;
...
int CryptoImpl::generateRsaKeypair(EVP_PKEY **keypair) {
...
int CryptoImpl::generateAesKey(unsigned char **aesKey, unsigned char **aesIv) {
...
int CryptoImpl::rsaEncrypt(const unsigned char *message, size_t messageLength, unsigned char **encryptedMessage, unsigned char **encryptedKey,
...
int CryptoImpl::rsaDecrypt(unsigned char *encryptedMessage, size_t encryptedMessageLength, unsigned char *encryptedKey,
...
int CryptoImpl::aesEncrypt(const unsigned char *message, size_t messageLength, unsigned char **encryptedMessage) {
  // Allocate memory for everything
  size_t blockLength = 0;
  size_t encryptedMessageLength = 0;

  if(messageLength + AES_BLOCK_SIZE >bufferSize){
      delete buffer;
      bufferSize = messageLength + AES_BLOCK_SIZE ;
      buffer = new char[bufferSize] ;
  }
  *encryptedMessage = (unsigned char *)buffer ;
  if(encryptedMessage == NULL) {
...
int CryptoImpl::aesDecrypt(const unsigned char *encryptedMessage, size_t encryptedMessageLength, unsigned char **decryptedMessage) {
  // Allocate memory for everything
  size_t decryptedMessageLength = 0;
  size_t blockLength = 0;

  if(encryptedMessageLength >bufferSize){
      delete buffer;
      bufferSize = encryptedMessageLength;
      buffer = new char[bufferSize] ;
  }

  *decryptedMessage = (unsigned char *)buffer /*new unsigned char[encryptedMessageLength]*/;
  if(*decryptedMessage == NULL) {
...
int CryptoImpl::getRemotePublicKey(unsigned char **publicKey) {
  BIO *bio = BIO_new(BIO_s_mem());
  PEM_write_bio_PUBKEY(bio, remotePublicKey);
  return bioToString(bio, publicKey);
}

int CryptoImpl::setRemotePublicKey(unsigned char *publicKey, size_t publicKeyLength) {
  BIO *bio = BIO_new(BIO_s_mem());
...
int CryptoImpl::getLocalPublicKey(unsigned char **publicKey) {
  BIO *bio = BIO_new(BIO_s_mem());
  PEM_write_bio_PUBKEY(bio, localKeypair);
  return bioToString(bio, publicKey);
}

int CryptoImpl::getLocalPrivateKey(unsigned char **privateKey) {
  BIO *bio = BIO_new(BIO_s_mem());
  PEM_write_bio_PrivateKey(bio, localKeypair, NULL, NULL, 0, 0, NULL);
  return bioToString(bio, privateKey);
}

int CryptoImpl::getAesKey(unsigned char **aesKey) {
  *aesKey = this->aesKey;
  return AES_KEYLEN/8;
}

int CryptoImpl::setAesKey(unsigned char *aesKey, size_t aesKeyLength) {
...
int CryptoImpl::getAesIv(unsigned char **aesIv) {
  *aesIv = this->aesIv;
  return AES_KEYLEN/8;
}

int CryptoImpl::setAesIv(unsigned char *aesIv, size_t aesIvLength) {
...
int CryptoImpl::writeKeyToFile(FILE *file, int key) {
  switch(key) {
    case KEY_SERVER_PRI:
...
int CryptoImpl::bioToString(BIO *bio, unsigned char **string) {
